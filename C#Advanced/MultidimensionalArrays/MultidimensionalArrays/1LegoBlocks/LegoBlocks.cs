...
namespace _1LegoBlocks
{
    class LegoBlocks
    {
        static void Main(string[] args)
        {
            ...
            var secondMatrix = ReadInput(rows)
                .Select(r => r.Reverse().ToArray())
                .ToArray();
        ...
        private static void PrintResult(int[][] firstMatrix, int[][] secondMatrix)
        {
            ...
            {
                Console.WriteLine($"The total number of cells is: {CellCount(firstMatrix, secondMatrix)}");
            }
        }

        private static object CellCount(int[][] firstMatrix, int[][] secondMatrix)
        {
            var numberOfCells = 0;
